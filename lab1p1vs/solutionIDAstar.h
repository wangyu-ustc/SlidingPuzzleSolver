...
status solutionIDAstar::solve(const status& s)
{
    ...
    while (d_limit < INT_MAX)
    {
        ...
        while (!list.empty())
        {
            ...
            n = list.top();
            list.pop();
            if (n->is_deprecated) { delete n; cnt_deprecated--; continue; }
            if (cnt % 100000 == 0)
            {
                std::cout << "Deprecated/Total = " << cnt_deprecated << "/" << list.size() << std::endl;
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (auto act : actions)
                    {
                        ...
                        if ((mcheck = list_map.find(*m)) != list_map.end())  //已经在open_list中
                        {
                            if (m->g < mcheck->second->g)        //比之前的好，更新 g 值
                            {
                                cnt_deprecated++;
                                mcheck->second->deprecate();    //由于priority_queue不能改变，设置原节点废弃，留在open_list中
